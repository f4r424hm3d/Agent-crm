...
// For simplicity, we can just copy the data here or read it from a file
// Since country.md is a TS file, we can extract the object
const countriesData = {
...
const seedCountries = async () => {
    try {
        ...
        // Clear existing country details
        await CountryDetail.deleteMany({});
        console.log('🧹 Cleared existing CountryDetail collection');

        const countriesToInsert = Object.entries(countriesData).map(([code, data]) => ({
            code,
            name: data.name,
            phone: data.phone
        }));

        await CountryDetail.insertMany(countriesToInsert);
        console.log(`✅ Successfully seeded ${countriesToInsert.length} countries`);
